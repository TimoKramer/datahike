...
(d/q '[:find ?mem ?file ?level ?pg :in $mem-db $file-db $level-db $pg-db :where [$mem-db ?e0 :name ?mem] [$file-db ?e1 :name ?file]
       [$level-db ?e2 :name ?level] [$pg-db ?e3 :name ?pg]]
     (d/db mem-conn)
     (d/db file-conn)
     (d/db level-conn)
     (d/db pg-conn))
