...
(def schema
  [{:db/ident       :contributor/name
   ...
   {:db/ident       :repository/tags
    :db/valueType   :db.type/ref
    :db/cardinality :db.cardinality/many
    :db/doc         "the repository's tags"} {:db/ident :language/clojure} {:db/ident :language/rust}])
...
(d/transact conn
            [{:contributor/name  "alice"
              :contributor/email "alice@exam.ple"}])
...
(d/transact conn
            [{:db/id            [:contributor/name "bob"]
              :contributor/name "bobby"}])
    ...
    (pull ?e
          [*
           {:repository/contributors [*]
            :repository/tags         [*]}]) :where [?e :repository/name ?n]])
