...
(defn pod-release
  ...
  (let [version (version/string repo-config)
    ...
    (if ssh-key
      (spit (str home "/.ssh/registry-key") ssh-key)
      (binding [*out* *err*]
        (println "Could not find ssh key")
        (System/exit 1)))
    (println (:err (b/process {:command-args ["ssh-add" (str home "/.ssh/pod-registry")] :out :capture :err :capture})))
    ...
    (let [url "https://api.github.com/repos/timokramer/pod-registry/pulls"]
