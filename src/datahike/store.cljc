...
(defmethod default-config :default
  [{:keys [backend]
    :as   config}]
...
(defmethod delete-store :mem
  [{:keys [id]}]
  (swap! memory dissoc id))

(defmethod connect-store :mem
  [{:keys [id]}]
  (@memory id))

(defmethod scheme->index :mem
  [_]
  :datahike.index/hitchhiker-tree)

(defmethod default-config :mem
  [{:keys [id]
    :as   config}]
  (merge
...
(s/def ::mem (s/keys :req-un [:datahike.store.mem/backend :datahike.store.mem/id]))
...
(defmethod scheme->index :file
  [_]
  :datahike.index/hitchhiker-tree)

(defmethod default-config :file
  [{:keys [id]
    :as   config}]
  (merge
...
(s/def ::file (s/keys :req-un {:datahike.store.file/backend :datahike.store.file/path}))
