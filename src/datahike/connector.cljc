...
(extend-protocol IConfiguration
  ...
  clojure.lang.IPersistentMap
    ...
    (-connect [config]
      (let [config (dc/load-config config)
            ...
            _ (when-not raw-store
                (dt/raise "Backend does not exist."
                          {:type   :backend-does-not-exist
                           :config config}))
            ...
            _ (when-not stored-db
                (ds/release-store store-config store)
                (dt/raise "Database does not exist."
    ...
    (-create-database [config & deprecated-config]
      (let [{:keys [keep-history? initial-tx]
             :as   config}
            ...
            _ (when stored-db
                (dt/raise "Database already exists."
                          {:type   :db-already-exists
                           :config store-config}))
            {:keys [eavt aevt avet temporal-eavt temporal-aevt temporal-avet schema rschema config max-tx]} (db/empty-db nil config)
            backend (kons/->KonserveBackend store)]
        (<?? S
             (k/assoc-in store
               [:db]
