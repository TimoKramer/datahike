(ns ^:no-doc datahike.query-v3
  (:require
    ...
    #?@(:cljs [datalog.parser.type :refer
               [BindColl BindIgnore BindScalar BindTuple Constant DefaultSrc Pattern RulesVar SrcVar Variable Not Or And Predicate
                PlainSymbol]])
  ...
  #?(:clj (:import
            [datalog.parser.type BindColl BindIgnore BindScalar BindTuple Constant DefaultSrc Pattern RulesVar SrcVar Variable Not Or And
             Predicate PlainSymbol]
...
#?(:clj (defmethod print-method ArrayRelation
          [rel w]
          (pr-rel rel w)))
...
#?(:clj (defmethod print-method CollRelation
          [rel w]
          (pr-rel rel w)))
...
(defn- join-tuples
  [rel1 t1 idxs1 rel2 t2 idxs2 arity target-idxs1 target-idxs2]
...
(defn- bind!
  ...
  (condp instance? binding
    ...
    (if (not (db/seqable? source))
      (raise "Cannot bind value "
             source
             " to collection "
             (dpi/get-source binding)
             {:error   :query/binding
              :value   source
              :binding (dpi/get-source binding)})
    ...
    (let [bindings (:bindings binding)]
      ...
      (when (< (count source) (count bindings))
        (raise "Not enough elements in a collection "
               source
               " to bind tuple "
               (dpi/get-source binding)
               {:error   :query/binding
...
(defn resolve-ins [context bindings values]
  (when (not= (count bindings) (count values))
    (raise "Wrong number of arguments for bindings " (mapv dpi/get-source bindings)
           ", "                                      (count bindings)
           " required, "                             (count values)
           " provided"
           {:error   :query/binding
            :binding (mapv dpi/get-source bindings)}))
...
(defn resolve-pattern-db [db clause]
  ...
  (let [pattern        (:pattern clause)
        search-pattern (mapv #(when (instance? Constant %)
                                (:value %))
                             pattern)
...
(defn- matches-pattern?
  [idxs tuple] ;; TODO handle repeated vars
  ;;   (when-not (db/seqable? tuple)
  ;;     (raise "Cannot match pattern " (dpi/get-source clause) " because tuple is not a collection: " tuple
  ;;            {:error :query/where, :value tuple, :binding (dpi/get-source clause)}))
  ;;   (when (< (count tuple) (count (:pattern clause)))
  ;;     (raise "Not enough elements in a relation tuple " tuple " to match " (dpi/get-source clause)
  ;;            {:error :query/where, :value tuple, :binding (dpi/get-source clause)}))
  (reduce-kv
    (fn [_ i v]
...
(defn resolve-pattern-coll [coll clause]
  (when-not (db/seqable? coll)
    (raise "Cannot match by pattern "
           (dpi/get-source clause)
           " because source is not a collection: "
           coll
           {:error   :query/where
            :value   coll
            :binding (dpi/get-source clause)}))
  (let [pattern (:pattern clause)
        idxs    (->> (map #(when (instance? Constant %1)
                             [%2 (:value %1)])
                          pattern
                          (range))
                     (remove nil?)
...
(defn subtract-from-rel [rel syms exclude-key-set]
  (let [key-fn1 (key-fn rel syms)
        pred    (fn [t1]
                  (contains? exclude-key-set (key-fn1 t1)))]
...
(comment
  ...
  (let [query  '[:find ?lid ?status ?starttime ?endtime (min ?paid) (distinct ?studentinfo) ?lgid :in $ ?tid ?week ?list :where
                 [?lid :lesson/teacherid ?tid] [?lid :lesson/week ?week] [?lid :lesson/lessongroupid ?lgid]
                 [?eid :enrollment/lessongroup_id ?lgid] [?eid :enrollment/student_id ?sid] [?iid :invoice/enrollment_id ?eid]
                 [?sid :student/firstname ?fname] [?sid :student/lastname ?lname] [?iid :invoice/paid ?paid] [?lid :lesson/status ?status]
                 [?lid :lesson/starttime ?starttime] [?lid :lesson/endtime ?endtime] [(?list ?sid ?fname ?lname) ?studentinfo]]
        parsed (parse query)]
  ...
  (do
    ...
    (bench "q2 const"
           '[:find ?e :where [?e :name "Ivan"] [?e :age 1]]
           db)

    #_(bench "q2 const in"
             '[:find ?e :in $ ?n :where [?e :name "Ivan"] [?e :age ?n]]
             db
             1)))
