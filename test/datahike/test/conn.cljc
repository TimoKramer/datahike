...
(def schema
  (merge db/implicit-schema
         {:aka {:db/cardinality :db.cardinality/many}}))

(def datoms #{(d/datom 1 :age 17) (d/datom 1 :name "Ivan")})
...
(deftest test-reset-conn!
  (let [conn    (d/conn-from-datoms datoms schema)
        ...
        datoms' #{(d/datom 1 :age 20) (d/datom 1 :sex :male)}
    ...
    (let [{:keys [db-before db-after tx-data tx-meta]} @report]
      ...
      (is (= :meta tx-meta))
      (is (= [[1 :age 17 false] [1 :name "Ivan" false] [1 :age 20 true] [1 :sex :male true]]
