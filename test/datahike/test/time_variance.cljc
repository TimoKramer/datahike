  ...
  (let [uri  "datahike:mem://test-base-history"
    ...
    (testing "find source transaction of retracted values"
      (is (= #{[25 true] [25 false] [30 true] [30 false]}
             (d/q '[:find ?a ?op :in $ ?e :where [?e :age ?a ?t ?op] [?t :db/txInstant ?d]]
                  (d/history @conn)
                  [:name "Alice"]))))))
  ...
  (let [uri  "datahike:mem://test-historical-queries"
    ...
    (testing "get all values before specific time"
      (let [_             (d/transact conn
            ...
            query         '[:find ?a :in $ ?e ?fd :where [?e :age ?a ?tx] [?tx :db/txInstant ?t] [(before? ?t ?fd)]]
            query-with-<  '[:find ?a :in $ ?e ?fd :where [?e :age ?a ?tx] [?tx :db/txInstant ?t] [(< ?t ?fd)]]]
        (is (= #{[35]}
