...
(def test-datoms
  (->>
    [[1 :name "Petr"] [1 :aka "Devil"] [1 :aka "Tupen"] [2 :name "David"] [3 :name "Thomas"] [4 :name "Lucy"] [5 :name "Elizabeth"]
     [6 :name "Matthew"] [7 :name "Eunan"] [8 :name "Kerri"] [9 :name "Rebecca"] [1 :child 2] [1 :child 3] [2 :father 1] [3 :father 1]
     [6 :father 3] [10 :name "Part A"] [11 :name "Part A.A"] [10 :part 11] [12 :name "Part A.A.A"] [11 :part 12] [13 :name "Part A.A.A.A"]
     [12 :part 13] [14 :name "Part A.A.A.B"] [12 :part 14] [15 :name "Part A.B"] [10 :part 15] [16 :name "Part A.B.A"] [15 :part 16]
     [17 :name "Part A.B.A.A"] [16 :part 17] [18 :name "Part A.B.A.B"] [16 :part 18]]
    (map (fn [[e a v]]
           (d/datom e a v tx0)))))
...
(deftest test-pull-attr-spec
  ...
  (is (= [{:name "Petr"} {:name "Elizabeth"} {:name "Eunan"} {:name "Rebecca"}]
...
(deftest test-pull-reverse-attr-spec
  ...
  (testing "Reverse non-component references yield collections"
    ...
    (is (= {:name    "Petr"
            :_father [{:db/id 2} {:db/id 3}]}
           (d/pull test-db '[:name :_father] 1)))

    (is (= {:name    "Thomas"
            :_father [{:name "Matthew"}]}
           (d/pull test-db '[:name {:_father [:name]}] 3)))

    (is (= {:name    "Petr"
            :_father [{:name "David"} {:name "Thomas"}]}
...
(deftest test-pull-component-attr
  (let [parts {:name "Part A"
        ...
        recdb (d/init-db
                ...
                test-schema)
        mutdb (d/init-db
                (concat test-datoms [(d/datom 12 :part 10) (d/datom 12 :spec 10) (d/datom 10 :spec 13) (d/datom 13 :spec 12)])
                test-schema)]
...
(deftest test-pull-limit
  (let [db (d/init-db
             (concat
               test-datoms
               [(d/datom 4 :friend 5) (d/datom 4 :friend 6) (d/datom 4 :friend 7) (d/datom 4 :friend 8)]
               (for [idx (range 2000)]
...
(deftest test-pull-recursion
  (let [db      (-> test-db
                    (d/db-with [[:db/add 4 :friend 5] [:db/add 5 :friend 6] [:db/add 6 :friend 7] [:db/add 7 :friend 8] [:db/add 4 :enemy 6]
                                [:db/add 5 :enemy 7] [:db/add 6 :enemy 8] [:db/add 7 :enemy 4]]))
        friends {:db/id  4
                 :name   "Lucy"
                 :friend [{:db/id  5
                           :name   "Elizabeth"
    ...
    (let [db (d/db-with db [[:db/add 8 :friend 4]])]
      (testing "Cycles are handled by returning only the :db/id of entities which have been seen before"
        (is (= (update-in friends
                          (take 8 (cycle [:friend 0]))
                          assoc
                          :friend
                          [{:db/id  4
                            :name   "Lucy"
...
(deftest test-dual-recursion
  (let [empty (d/empty-db {:part {:db/valueType :db.type/ref}
                           :spec {:db/valueType :db.type/ref}})]
    (let [db (d/db-with empty [[:db/add 1 :part 2] [:db/add 2 :part 3] [:db/add 3 :part 1] [:db/add 1 :spec 2] [:db/add 2 :spec 1]])]
...
(deftest test-deep-recursion
  (let [start  100
        ...
        txd    (mapcat
                 (fn [idx]
                   [(d/datom idx :name (str "Person-" idx)) (d/datom (dec idx) :friend idx)])
