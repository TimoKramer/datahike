...
(deftest test-explode
  (doseq [coll [["Devil" "Tupen"] #{"Devil" "Tupen"} '("Devil" "Tupen") (to-array ["Devil" "Tupen"])]]
    (testing coll
      (let [conn (d/create-conn {:aka  {:db/cardinality :db.cardinality/many}
        ...
        (d/transact! conn
                     [{:db/id -1
                       ...
                       :aka   coll
                       :also  "ok"}])
        (is (= (d/q '[:find ?n ?a :where [1 :name ?n] [1 :age ?a]] @conn)
               #{["Ivan" 16]}))
  ...
  (let [db0 (d/empty-db {:children {:db/valueType   :db.type/ref
    ...
    (let [db (d/db-with db0
                        [{:db/id    -1
                         ...
                         {:db/id -3
                          :name  "Evgeny"}])]
      (is (= (d/q '[:find ?n :where [_ :children ?e] [?e :name ?n]] db)
             #{["Petr"] ["Evgeny"]})))
