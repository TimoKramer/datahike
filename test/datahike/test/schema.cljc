...
(def find-name-q '[:find ?n :where [_ :name ?n]])

(def find-schema-q '[:find ?n ?vt ?c :where [?e :db/ident ?n] [?e :db/valueType ?vt] [?e :db/cardinality ?c]])
...
(deftest test-ident
  (testing "use db/ident as enum"
    (let [cfg    "datahike:mem://test-ident"
          schema [{:db/ident :important} {:db/ident :archive}
                  {:db/ident       :message
                   :db/valueType   :db.type/string
                   :db/cardinality :db.cardinality/one}
                  {:db/ident       :tag
