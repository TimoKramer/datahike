...
(deftest test-filter-db
  (let [empty-db    (d/empty-db {:aka {:db/cardinality :db.cardinality/many}})
        db          (-> empty-db
                        (d/db-with [{:db/id    1
                                    ...
                                    {:db/id    3
                                     ...
                                     :aka      ["II"]
                                     :password "<UNKWOWN>"}]))
        remove-pass (fn [_ datom]
                      (not= :password (:a datom)))
        remove-ivan (fn [_ datom]
                      (not= 2 (:e datom)))
        long-akas   (fn [udb datom]
                      (or (not= :aka (:a datom))
                          ;; has just 1 aka
    ...
    (are [_db _res]
         (= (d/q '[:find ?v :where [_ :password ?v]] _db) _res)
         db
    ...
    (are [_db _res]
         (= (d/q '[:find ?v :where [_ :aka ?v]] _db) _res)
         db
    ...
    (let [db       (d/db-with (d/empty-db {})
                              [{:db/id 1
                               ...
                               {:db/id 3
                                ...
                                :age   12}])
          has-age? (fn [db datom]
                     (some? (:age (d/entity db (:e datom)))))
          adult?   (fn [db datom]
                     (>= (:age (d/entity db (:e datom))) 18))
          names    (fn [db]
                     (map :v (d/datoms db :aevt :name)))]
