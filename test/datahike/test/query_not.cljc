...
(deftest test-not
  (are [q res]
       (= (set (d/q (concat '[:find [?e ...] :where] (quote q)) @test-db))
          res)
...
(deftest test-not-join
  (are [q res]
       (= (d/q (concat '[:find ?e ?a :where] (quote q)) @test-db)
          res)
       [[?e :name] [?e :age ?a]
        (not-join [?e]
                  [?e :name "Oleg"]
...
(deftest test-default-source
  (let [db1 (d/db-with (d/empty-db)
                       [[:db/add 1 :name "Ivan"] [:db/add 2 :name "Oleg"]])
        db2 (d/db-with (d/empty-db)
                       [[:db/add 1 :age 10] [:db/add 2 :age 20]])]
    (are [q res]
         (= (set (d/q (concat '[:find [?e ...] :in $ $2 :where]
                              (quote q))
                      db1
                      db2))
            res)
...
(deftest test-impl-edge-cases
  (are [q res]
       ...
       ;; 2 rels \ 2 rels
       [:find ?e ?e2 :where [?e :name "Ivan"] [?e2 :name "Ivan"]
        (not [?e :age 10]
             [?e2 :age 20])]
       #{[2 1] [6 5] [1 1] [2 2] [5 5] [6 6] [2 5] [1 5] [2 6] [6 1] [5 1] [6 2]}

       ;; 2 rels \ rel + const
       [:find ?e ?e2 :where [?e :name "Ivan"] [?e2 :name "Oleg"]
        (not [?e :age 10]
             [?e2 :age 20])]
       #{[2 3] [1 3] [2 4] [6 3] [5 3] [6 4]}

       ;; 2 rels \ 2 consts
       [:find ?e ?e2 :where [?e :name "Oleg"] [?e2 :name "Oleg"]
        (not [?e :age 10]
             [?e2 :age 20])]
       #{[4 3] [3 3] [4 4]}
  ))
