...
(deftest return-keys-test
  ...
  (is (= [{:entity 4
          ...
          {:entity 3
           :name   "Alice"
           :age    20}]
         (d/q '[:find ?e ?n ?a :keys entity name age :where [?e :name ?n] [?e :age ?a]]
              @conn))))
...
(deftest return-syms-test
  ...
  (is (= [{'entity 4
          ...
          {'entity 3
           'name   "Alice"
           'age    20}]
         (d/q '[:find ?e ?n ?a :syms entity name age :where [?e :name ?n] [?e :age ?a]]
              @conn))))
...
(deftest return-strs-test
  ...
  (is (= [{"entity" 4
          ...
          {"entity" 3
           "name"   "Alice"
           "age"    20}]
         (d/q '[:find ?e ?n ?a :strs entity name age :where [?e :name ?n] [?e :age ?a]]
              @conn))))
